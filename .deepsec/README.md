...
Currently configured project: `app.bringa.io` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/app.bringa.io/SETUP.md` to fill in
   `data/app.bringa.io/INFO.md`.
...
`--project-id app.bringa.io` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/app.bringa.io/`.
