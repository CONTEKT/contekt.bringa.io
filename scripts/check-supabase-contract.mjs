...
export function checkSupabaseContract({ schema, config }) {
  ...
  const requiredVisibilitySql = [
    ...
    ["admins can still view all items", "(select public.is_admin()) OR"],
  ...
  const requiredDeletionReviewSql = [
    ...
    ["deletion request owner read handles retained audit rows", "CREATE POLICY \"Users can view own deletion requests\" ON public.account_deletion_requests FOR SELECT TO authenticated USING ("],
  ];
