...
- `/admin/dashboard`: item counts, visibility signals, borrowed-first item list, pending queue counts, recent activity, recent uploads, latest backup run status when `backup_runs` exists, and system-readiness links for config, Supabase, Storage, local backend setup, backups, docs, and Telegram.
