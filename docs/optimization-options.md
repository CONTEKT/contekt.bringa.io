...
- Security maintenance drills: run the new agent security-maintenance workflow after larger updates and record gaps that are not covered by secret scans, static checks, Supabase advisors, browser evidence, or restore drills. Impact: keeps security review repeatable for humans, agents, and forks. Uncertainty/research: decide when live advisor output is mandatory versus optional for fork-only changes.
