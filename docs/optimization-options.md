...
- Item visibility lifecycle: wire prepared visibility states through user/admin RPCs, list filters, counts, and UI affordances. Impact: users retain agency while admins keep moderation control. Side effect: lists, counts, filters, and RLS need consistent semantics.
...
- Item versioning: populate prepared `item_versions` through item create/update/restore flows with admin restore-by-republish behavior. Impact: auditability and safe moderation. Uncertainty/research: define whether trigger-based or RPC-explicit version creation is easier to audit before enabling it.
...
- Admin item control: build on the dashboard stats and prepared visibility fields with per-user item views, hide/unhide reason flows, and restore visibility through moderation state. Impact: operator efficiency. Side effect: must avoid bypassing ownership/version rules.
- Moderation follow-through: build on suggestion/flag creation and review RPCs with accepted-suggestion application, item/image-specific queues, seen-state, history, and Telegram dedupe/mute policy. Impact: community quality without broad browser writes. Uncertainty/research: notification privacy and review-state semantics need live-project and operator review.
...
- User data export and deletion processing: extend the prepared user JSON export and deletion-request RPC with admin review, Auth deletion, Storage cleanup/export, suggestions/flags, and image metadata. Impact: trust and portability. Uncertainty/research: requires Supabase Auth/Storage policy review and legal/operator retention decisions.
...
- Major dependency upgrades: review `lucide-react`, `@types/node`, ESLint, and TypeScript major updates with current official docs before upgrading. Impact: keeps tooling current without avoidable breakage. Uncertainty/research: confirm runtime Node policy, lint migration path, compiler changes, and visual icon regressions.
