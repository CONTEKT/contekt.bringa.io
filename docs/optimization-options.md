...
- Test strategy: extend focused tests beyond config generation, OAuth redirect helpers, app-config helpers, admin route gates, media policy helpers, protected-route decisions, and Supabase contract checker behavior to more critical UI states. Impact: safer refactors. Side effect: respect the user's preference for agentic browser skills before adding browser-test packages.
