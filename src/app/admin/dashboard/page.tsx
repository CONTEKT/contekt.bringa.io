...
import {
  buildAdminSystemHealthItems,
  type AdminBackupRun,
  type AdminSystemHealthItemKey,
...
export default function AdminDashboardPage() {
  ...
  const [recentActivity, setRecentActivity] =
    useState<AdminRecentActivity | null>(null);
  const [latestBackupRun, setLatestBackupRun] = useState<
    AdminBackupRun | null | undefined
  >(undefined);
  ...
  const adminGate = buildAdminRouteGate({
    adminLoading,
    isAdmin,
  ...
  const stats = useMemo(() => {
    ...
    const hidden = items.filter((item) =>
      hiddenStates.has(item.visibility_state || ""),
    ).length;
    const pendingVisible = items.filter(
      (item) => item.visibility_state === "pending_visible",
    ).length;
  ...
  const statCards = [
    ...
    { label: "With images", value: stats.withImages, icon: ImageIcon },
    {
      ...
      value: queueCounts.pendingSuggestions ?? "—",
      icon: Sparkles,
    },
    {
      label: "Deletion requests",
      value: queueCounts.openDeletionRequests ?? "—",
      icon: Trash2,
    },
    {
      label: "Pending users",
      value: queueCounts.pendingUsers ?? "—",
      icon: Users,
    },
  ];
  ...
  useEffect(() => {
    const fetchAllItems = async () => {
      try {
        ...
        const borrowedItems = itemsData.filter(
          (i) => i.status === "borrowed" && i.borrowed_by,
        );
        const borrowedItemIds = borrowedItems.map((i) => i.id);
        const borrowerIds = borrowedItems
          .map((i) => i.borrowed_by)
        ...
        // The user requested: "On the top there
        //  are the borrowed items and then the not borrowed ones."
  ...
  useEffect(() => {
    const fetchQueueCounts = async () => {
      try {
        const [suggestionsRes, flagsRes, deletionRequestsRes, profilesRes] =
          await Promise.all([
    ...
    const fetchLatestBackupRun = async () => {
      try {
        const { data, error } = await supabase
          .from("backup_runs")
          .select(
            "status,finished_at,table_count,table_rows,storage_bucket_count,storage_object_count,storage_bytes,auth_users_exported,auth_user_count",
          )
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Items, queues, and system readiness
              </p>
            </div>
          ...
          <section className="grid gap-3 lg:grid-cols-2">
            <div className="rounded-lg border bg-card p-3">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="text-xs text-muted-foreground">
                  {recentActivity?.borrowReturns.length ?? 0} records
                </span>
              </div>
                  ...
                  {recentActivity.borrowReturns.map((event) => (
                    ...
                    >
                      ...
                      <span className="shrink-0 text-xs text-muted-foreground">
                        {event.kind === "borrowed" ? "Borrowed" : "Returned"}{" "}
            ...
            <div className="rounded-lg border bg-card p-3">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="text-xs text-muted-foreground">
                  {recentActivity?.uploads.length ?? 0} records
                </span>
              </div>
                ...
                <div className="mt-3 flex flex-col gap-2">
                  {recentActivity.uploads.map((event) => (
                    ...
                    >
                      ...
                      <span className="shrink-0 text-xs text-muted-foreground">
                        Image {formatDate(event.occurredAt)}
                      </span>
                    </Link>
                  ))}
                </div>
                        ...
                        {item.status === "borrowed" && (
                          <div className="mt-1 flex flex-col text-xs font-medium text-amber-600 dark:text-amber-500">
                            <span className="truncate">
                              Borrowed by: {item.borrowerName || "Unknown"}
                            </span>
                            {item.borrowedDate && (
                              <span className="text-muted-foreground opacity-80">
                                {
                                  new Date(item.borrowedDate)
                                    .toISOString()
                                    .split("T")[0]
                                }
                              </span>
