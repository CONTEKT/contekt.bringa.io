...
- Local Supabase seed growth: build on `pnpm seed:local-supabase` with optional flags for reset, richer media fixtures, moderation queues, deletion requests, and agent personas once real local-backend work shows repeated friction. Impact: makes schema/RLS/Auth/Storage development pleasant without hosted dev projects. Uncertainty/research: keep the script localhost-only and avoid turning fixtures into production policy.
