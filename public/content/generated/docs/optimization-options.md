...
- Supabase contract alignment: reconcile `supabase/schema.sql`, migrations, client code, Storage buckets, edge functions, and generated docs into one enforceable contract. Impact: security, reliability, and maintainability. Uncertainty/research: requires Supabase MCP or server-side maintenance access, but real row contents must not be inspected without explicit user approval.
