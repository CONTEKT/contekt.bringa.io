...
- For local browser testing, run `pnpm dev:docker`, which regenerates config with local Docker overrides before starting Next.js. Sign in on `/login` with the seeded Admin or Member email/password panel, and restore generated config afterward with plain `pnpm generate:config` before committing.
